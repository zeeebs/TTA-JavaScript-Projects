...
const Calculator = {
    ...
    First_Operand: null,
    //this checks whether or not the second operand has been put in by the user
...
function Handle_Operator(Next_Operator) {
    ...
    //when an operator key is pressed we convert the current number displayed on the screen to a number then store the result in Calculator.First_Operand if it doesn't already exist
    const Value_of_Input = parseFloat(Display_Value);
    //checks if an operator already exists and if Wait_Second)Operand is true, updates operator an exists from the function
